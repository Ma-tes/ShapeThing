    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (processeID != 0)
            {
                ...
                for (int i = 0; i < roads.Symbols.Count; i++)
                {
                    Console.SetCursorPosition(roads.Positions[i].x, roads.Positions[i].y);
                    Console.Write(roads.Symbols[i]);
                }
                ...
                {
                    roads.Positions.Add(consoleMouseInput.CursorPosition);
                    roads.Symbols.Add(RoadHelper.GetRoadShape(RoadType.DefaultRoadTypes, RoadHelper.GetNeighborsList(consoleMouseInput.CursorPosition, roads.Positions)));
                }
